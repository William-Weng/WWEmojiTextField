...
public extension WWEmojiTextField {
    ...
    func configure(mapping: [String: String], convertRegex: Regex<(Substring, Substring)> = try! Regex("\\((\\w+)\\s$"), highlightRegex: Regex<(Substring, Substring)> = try! Regex("\\((\\w+)$")) {
...
private extension WWEmojiTextField {
    ...
    func convertToEmoji(textField: UITextField) {
        ...
        let result2 = textField._highlight(with: highlightRegex, mapping: mapping, color: .lightGray)
